...
class BundleHelper {
    private let bundleName = "Spotifyy"
    ...
    private init() {
        ...
        if let path = Bundle.main.path(forResource: bundleName, ofType: "bundle"),
           let b = Bundle(path: path) {
            self.bundle = b
            NSLog("[Spotifyy] Loaded bundle from main bundle: \(path)")
        } 
        // If not found, try locating in file system (jailbreak path)
        else {
            let jbPath = SpotifyyJBRootPath("/Library/Application Support/\(bundleName).bundle")
            if let b = Bundle(path: jbPath) {
                self.bundle = b
                NSLog("[Spotifyy] Loaded bundle from filesystem: \(jbPath)")
            } else {
                NSLog("[Spotifyy] ERROR: Could not find Spotifyy.bundle!")
                self.bundle = nil
            }
        }
        ...
        if let b = self.bundle, let enPath = b.path(forResource: "en", ofType: "lproj"),
           let enB = Bundle(path: enPath) {
            ...
            NSLog("[Spotifyy] WARNING: Could not load en.lproj from bundle")
            self.enBundle = nil
        }
    }
    ...
    func resolveConfiguration() throws -> ResolveConfiguration {
        ...
        guard let bundle = self.bundle,
            ...
            throw NSError(domain: "Spotifyy", code: 404, userInfo: [NSLocalizedDescriptionKey: "Configuration not found"])
        }
