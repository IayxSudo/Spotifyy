...
private let selectedKeyDefault = "SpotifyySelectedAppIconName"
...
struct SpotifyyAppIconPickerView: View {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(icon.id == selectedKey ? "iconSelected".localized : "iconTapToApply".localized)
                    ...
                    .foregroundColor(icon.id == selectedKey
                                     ? SpotifyySettingsView.spotifyAccentColor
                                     : .secondary)
            }
            Spacer()
    ...
    private func apply(_ icon: AppIconEntry) {
        ...
        UIApplication.shared.setAlternateIconName(icon.alternateName) { error in
            DispatchQueue.main.async {
                ...
                NSLog("[Spotifyy][AppIcon] setAlternateIconName(%@) failed: %@",
    ...
    ///   no space is already present — e.g. "SpotifyyV2" → "Spotifyy V2".
    ...
    static func prettifyIconKey(_ key: String, prettify: Bool = true) -> String {
        ...
        for i in chars.indices {
            ...
            if i > 0 {
                ...
                // Letter → digit boundary: "Spotifyy2" → "Spotifyy 2"
                if c.isNumber && prev.isLetter && !prevIsSpace {
                    spaced.append(" ")
                }

                // Digit → letter boundary: "2Spotifyy" → "2 Spotifyy"
