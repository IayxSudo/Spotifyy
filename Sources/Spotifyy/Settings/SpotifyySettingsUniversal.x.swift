...
class UniversalProfileSettingsSectionHook: ClassHook<NSObject> {
    ...
    private func openSpotifyySettingsFromHook() {
        ...
        button.addTarget(
            spotifyySettingsController,
            action: #selector(spotifyySettingsController.openRepositoryUrl(_:)),
            for: .touchUpInside
        )
        ...
        spotifyySettingsController.navigationItem.rightBarButtonItem = menuBarItem
        
        navigationController.pushViewController(spotifyySettingsController, animated: true)
    }
}

// MARK: - Global Helper to avoid Orion Hooking Issues with setupSpotifyyButton
// This logic is moved outside the ClassHook so Orion doesn't try to find it as an Obj-C method on the target class.
func injectSpotifyyButton(into target: UIViewController) {
    NSLog("[Spotifyy] injectSpotifyyButton called for \(String(describing: type(of: target)))")
    ...
    if let rightItems = target.navigationItem.rightBarButtonItems {
        if rightItems.contains(where: { $0.tag == 1337 }) {
             NSLog("[Spotifyy] Button already exists (tag 1337)")
             return 
        }
    }

    NSLog("[Spotifyy] Creating and injecting button...")
    ...
    let action = UIAction { [weak target] _ in
        guard let target = target, let navigationController = target.navigationController else { 
            NSLog("[Spotifyy] Navigation controller not found")
            return 
        }
        
        NSLog("[Spotifyy] Opening SpotifyySettings...")
        ...
        let subAction = UIAction { [weak spotifyySettingsController] _ in
            spotifyySettingsController?.openRepositoryUrl(subButton)
        }
    ...
    NSLog("[Spotifyy] Button injected. Items count: \(items.count)")
}
...
class SettingsViewControllerHook: ClassHook<UIViewController> {
    ...
    func viewDidLoad() {
        orig.viewDidLoad()
        injectSpotifyyButton(into: target)
    }
...
class SettingsListViewControllerHook: ClassHook<UIViewController> {
    ...
    func viewDidLayoutSubviews() {
        ...
        injectSpotifyyInlineRow(into: target)
    }
}
...
func injectSpotifyyInlineRow(into vc: UIViewController) {
    ...
    guard let cv = findFirstCollectionView(in: vc.view) else {
        NSLog("[Spotifyy] inlineRow: no UICollectionView in view tree")
        return
    }
    if cv.viewWithTag(spotifyyInlineRowTag) != nil { return }

    let row = UIButton(type: .custom)
    row.tag = spotifyyInlineRowTag
    row.backgroundColor = .clear
    row.frame = CGRect(x: 0, y: -spotifyyInlineRowHeight, width: cv.bounds.width, height: spotifyyInlineRowHeight)
    ...
    title.text = spotifyyInlineRowTitle
    ...
    subtitle.text = "spotifyy_inline_subtitle_text".localized
    ...
    chevron.frame = CGRect(
        ...
        y: (spotifyyInlineRowHeight - chevSize) / 2,
        width: chevSize,
        height: chevSize
    )
...
private func pushSpotifyySettings(from vc: UIViewController) {
    ...
    func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        let checkBlock = {
            ...
            if let title = targetVC.title, settingsTitles.contains(title) {
                NSLog("[Spotifyy] Detected Settings via Title: \(className)")
                injectSpotifyyButton(into: targetVC)
                return
            }
            
            // Check class name
            if className.contains("Settings") && !className.contains("Spotifyy") {
                NSLog("[Spotifyy] Detected Settings via Class Name: \(className)")
                injectSpotifyyButton(into: targetVC)
                return
            }
        }
