...
struct SpotifyySettingsVersionView: View {
    ...
    private var isUpdateAvailable: Bool {
        ...
        return isNewerVersion(latestVersion, than: Spotifyy.version)
    }
        ...
        Section {
            if isUpdateAvailable {
                Link(
                    ...
                    destination: URL(string: "https://github.com/jaydenjcpy/SpotifyyReincarnated/releases")!
                )
            }
        } footer: {
            VStack(alignment: .leading) {
                Text("v\(Spotifyy.version) (build \(Spotifyy.buildNumber))")
