...
enum SponsorBlockReporter {
    static let userAgent = "Spotifyy/\(Spotifyy.version)"
    ...
    static func submitSegment(
        ...
        req.setValue("Spotifyy-SponsorBlock/1", forHTTPHeaderField: "X-CLIENT-NAME")

        let payloadStr = String(data: payload, encoding: .utf8) ?? "<binary>"
        NSLog("[Spotifyy][SB][SUBMIT] POST %@ payload=%@", url.absoluteString, payloadStr)
        ...
        session.dataTask(with: req) { data, resp, err in
            ...
            NSLog("[Spotifyy][SB][SUBMIT] <- %d body=%@", http.statusCode, bodyStr ?? "<nil>")
