...
private func upsellServiceLog(_ message: String) {
    NSLog("[Spotifyy][UpsellService] %@", message)
}
...
func activateUpsellServiceBlocker() {
    ...
    for (className, label, activate) in targets {
        guard let cls = NSClassFromString(className),
            ...
            NSLog("[Spotifyy][UpsellService] %@/load unavailable; skipping", label)
            continue
        }
        activate()
        activated += 1
        NSLog("[Spotifyy][UpsellService] %@ hook activated", label)
    }
    ...
    if let cls = NSClassFromString(SelfLoadingUpsellBannerViewKill.targetName) as? UIView.Type,
       class_getInstanceMethod(cls, viewSelector) != nil {
        ...
        activated += 1
        NSLog("[Spotifyy][UpsellService] Premium banner view fallback activated")
    } else {
        NSLog("[Spotifyy][UpsellService] Premium banner view unavailable; skipping")
    }

    NSLog("[Spotifyy][UpsellService] activated %d/%d compatible hooks",
