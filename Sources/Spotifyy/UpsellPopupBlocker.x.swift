...
class SPTEncorePopUpDialogModelHook: ClassHook<NSObject> {
    ...
    func initWithTitle(
        ...
        if !isSpotifyyPopupTitle(title) && containsUpsellText {
            markAsUpsell(model)
        }
        return model
    }
...
class SPTEncorePopUpPresenterHook: ClassHook<NSObject> {
    ...
    func presentPopUp(_ popUp: NSObject) {
        if isMarkedAsUpsell(popUp) {
            NSLog("[Spotifyy][UpsellBlock] Blocked popup captured from model")
            return
        }
        ...
        if isSpotifyyPopupTitle(title) {
            orig.presentPopUp(popUp)
            return
        }

        if isUpsellText(title) || isUpsellText(desc) {
            NSLog("[Spotifyy][UpsellBlock] Blocked popup — title=%@ desc=%@",
                  title ?? "(nil)", desc ?? "(nil)")
            return
        }
...
func activateUpsellPopupBlocker() {
    ...
    for (className, selectors, label, activate) in targets {
        guard let cls = NSClassFromString(className),
            ...
            NSLog("[Spotifyy][UpsellBlock] %@ unavailable; skipping", label)
            continue
        }
        activate()
        activated += 1
        NSLog("[Spotifyy][UpsellBlock] %@ activated", label)
    }

    NSLog("[Spotifyy][UpsellBlock] activated %d/%d compatible hooks",
