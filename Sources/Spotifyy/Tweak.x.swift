...
let tweakInitTime: Date = {
    if let existing = getenv("SPOTIFYY_BOOT_TIME"),
       let interval = Double(String(cString: existing)) {
        return Date(timeIntervalSince1970: interval)
    }
    let now = Date()
    setenv("SPOTIFYY_BOOT_TIME", "\(now.timeIntervalSince1970)", 1)
    return now
}()
...
func activatePremiumPatchingGroup() {
    ...
    if Spotifyy.hookTarget == .lastAvailableiOS14 {
        IOS14PremiumPatchingGroup().activate()
    }
    else if Spotifyy.hookTarget == .v91 {
        ...
        if Spotifyy.hookTarget == .lastAvailableiOS15 {
...
func activateSessionLogoutProtection(minimal: Bool) {
    func log(_ msg: String) {
        NSLog("[Spotifyy][SessionProtect] %@", msg)
    }
...
@inline(__always)
func spotifyyBreadcrumb(_ label: String) {
    let path = NSTemporaryDirectory() + "spotifyy_boot.txt"
...
struct Spotifyy: Tweak {
    ...
    static var hookTarget: VersionHookTarget {
        ...
        NSLog("[Spotifyy] Detected Spotify version: \(version)")
    ...
    // activates automatically during its init sequence, BEFORE `Spotifyy.init()` runs -
    ...
    static func handleError(_ error: OrionHookError) {
        ...
        NSLog("[Spotifyy][OrionError] Hook activation failed (non-fatal): %@", description)
        writeDebugLog("[ORION ERROR] \(description)")
        spotifyyBreadcrumb("Orion hook activation failed (continuing): \(description)")
        // Deliberately NOT calling handleErrorDefault(error) here - that is what fatalErrors.
    }

    init() {
        spotifyyBreadcrumb("Tweak init() entered")
        ...
        if spotifyyEnvFlag("SPOTIFYY_DISABLE_ALL") {
            spotifyyBreadcrumb("SPOTIFYY_DISABLE_ALL=1 -> returning without hooks")
            return
        }
        ...
        // other Spotifyy path is disabled.
        activateSpotifyyPremiumForce()

        activateSpotifyyCrossfadeForce()

        // TESTING: extended ad blocker (NPV/lyrics ad, home brand-ads, in-stream).
        activateSpotifyyAdBlockerExtended()
        ...
        if NSClassFromString("HUBViewModelBuilderImplementation") != nil {
            ...
            NSLog("[Spotifyy] AdBlockerGroup activated")
        }

        // activateSpotifyyFlexGesture()
        ...
        writeDebugLog("=== Spotifyy \(Spotifyy.version) (build \(Spotifyy.buildNumber)) starting ===")
        writeDebugLog("[INIT] Spotify: \(spotifyVersion) (build \(spotifyBuild))")
        writeDebugLog("[INIT] iOS: \(iosVersion), Device: \(deviceModel)")
        writeDebugLog("[INIT] Hook target: \(Spotifyy.hookTarget)")
        ...
        if Spotifyy.hookTarget == .v91 {
            ...
            NSLog("[Spotifyy] Initialization complete for 9.1.x")
            TrueShuffleHook.install()
            activateSpotifyyProbes()
            activateSponsorBlock()
            activateKaraokeHooks()
            // Shared player event stream: sleep timer + listening stats.
            activateSpotifyyPlayerEvents()
            return
        }
        ...
        // Shared player event stream: sleep timer + listening stats. Safe to
        // call unconditionally — it only registers an observer, and each
        // feature checks its own enabled state when it reacts.
        activateSpotifyyPlayerEvents()
    }
}
